...
<body class="bg-gray-50 text-gray-900 min-h-screen">
  ...
  <main class="pt-20 px-4 pb-8">
    <div class="max-w-md mx-auto space-y-6">
      ...
      <div class="grid grid-cols-2 gap-4">
        ...
        <div class="bg-white rounded-lg p-4 stat-shadow">
          <div class="text-center">
            <div class="text-2xl font-bold text-gray-900">{{ $stats['monthlyIncome'] }}</div>
            <div class="text-sm text-gray-600">Monthly Income</div>
          </div>
        </div>
        <div class="bg-white rounded-lg p-4 stat-shadow">
          <div class="text-center">
            <div class="text-2xl font-bold text-gray-900">{{ $stats['bookedProperties'] }}</div>
      ...
      <div class="bg-white rounded-lg card-shadow p-4">
        ...
        <div class="space-y-3">
          <a href="{{ route('properties.create') }}" class="block w-full bg-green-600 hover:bg-green-700 text-white py-3 rounded-lg font-medium transition-colors duration-200 text-center">
            + Add New Property
          </a>
          <a href="{{ route('owner.properties') }}" class="block w-full bg-green-700 hover:bg-green-800 text-white py-3 rounded-lg font-medium transition-colors duration-200 text-center">
            View All Properties
          </a>
          <a href="{{ route('bookings.index') }}" class="block w-full bg-green-800 hover:bg-green-900 text-white py-3 rounded-lg font-medium transition-colors duration-200 text-center">
            Manage Bookings
          </a>
        </div>
      </div>
      ...
      <div class="bg-white rounded-lg card-shadow p-4">
        <div class="flex items-center justify-between mb-4">
          ...
          <a href="{{ route('owner.properties') }}" class="text-green-600 text-sm font-medium hover:text-green-700">View All</a>
        </div>
        ...
        <div class="space-y-3">
          @if(isset($recentProperties) && $recentProperties->count() > 0)
            @foreach($recentProperties as $property)
              ...
              <div class="flex items-center space-x-3 p-3 bg-gray-50 rounded-lg">
                <img src="{{ $property->image ? asset('storage/'.$property->image) : 'https://images.unsplash.com/photo-1586023492125-27b2c045efd7?ixlib=rb-4.0.3&auto=format&fit=crop&w=100&q=80' }}" 
                     alt="{{ $property->title }}" class="w-12 h-12 rounded-lg object-cover">
                <div class="flex-1">
                  <p class="text-sm font-medium text-gray-800">{{ Str::limit($property->title, 25) }}</p>
                  <p class="text-xs text-gray-600">₱{{ number_format($property->price_per_night) }}/night • Available</p>
                </div>
          ...
          @else
            ...
            <div class="text-center py-6">
              ...
              <a href="{{ route('properties.create') }}" class="text-green-600 hover:text-green-700 text-sm font-medium">
  ...
  <script>
    ...
    const monthlyIncome = {{ $stats['monthlyIncome'] === '₱0' ? 0 : (int) str_replace(['₱', ','], '', $stats['monthlyIncome']) }};
